...
public class AppointmentDAO {
	...
	public boolean updateAppointment(Appointment appointment) {
	    boolean f = false;
	    try {
	        String sql = "UPDATE appointment SET doctor_id=?, fullname=?, gender=?, age=?, email=?, phno=?, diseases=?, appoint_date=?, address=? WHERE id=?";
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ...
	        ps.setString(6, appointment.getPhNo());
	        ps.setString(7, appointment.getDiseases());
	        ps.setString(8, appointment.getAppoinDate());
	        ps.setString(9, appointment.getAddress());
	        ps.setInt(10, appointment.getId());

	        int i = ps.executeUpdate();
	        if (i == 1) {
	            f = true; // Update was successful
	        }
	...
	public boolean doesEmailExist(String email, int appointmentId) {
	    boolean exists = false;
	    try {
	        String sql = "SELECT COUNT(*) FROM appointment WHERE email = ? AND id != ?";
	        PreparedStatement ps = conn.prepareStatement(sql);
	...
	public boolean doesPhoneExist(String phNo, int appointmentId) {
	    boolean exists = false;
	    try {
	        String sql = "SELECT COUNT(*) FROM appointment WHERE phno = ? AND id != ?";
	        PreparedStatement ps = conn.prepareStatement(sql);
	        ps.setString(1, phNo);
	        ps.setInt(2, appointmentId);
	        ResultSet rs = ps.executeQuery();
