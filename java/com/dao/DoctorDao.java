...
public class DoctorDao {
	...
	public boolean checkEmailExists(String email, int docId) {
	    ...
	    try (PreparedStatement ps = conn.prepareStatement(sql)) {
	        ...
	        try (ResultSet rs = ps.executeQuery()) {
	            return rs.next();  // Returns true if another doctor with the same email exists
	        }
	    } catch (Exception e) {
	...
	public boolean editDoctorProfile(Doctor doctor) {
	    String sql = "UPDATE doctor SET full_name=?, dob=?, qualification=?, email=?, mobno=? WHERE id=?";
	    try (PreparedStatement ps = conn.prepareStatement(sql)) {
	        ...
	        return rowsAffected == 1;  // Return true if exactly one row was updated
	...
	public List<Doctor> getAllDoctors() {
	    ...
	    try {
	        String query = "SELECT * FROM doctor";  // Adjusted table name to 'doctor'
